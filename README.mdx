# Adobe Commerce Bold Checkout PayPal Integration

This module replaces all other Bold-hosted Checkout Flows with the PayPal Checkout Flow.

## Table of Contents

- [Adobe Commerce Bold Checkout PayPal Integration](#adobe-commerce-bold-checkout-paypal-integration)
...
- [Install and configure Bold Checkout on your Adobe Commerce store](https://developer.boldcommerce.com/guides/platform-integration/adobe-commerce/installation)
  - The Adobe Commerce Bold Checkout Integration module version must be >= `1.1.23`

## Installation

Install and enable the module on your store with the following commands:

1. Install the Bold Checkout Flow PayPal module:
...
After installing the module, enable the PayPal Checkout Flow from your Adobe Commerce admin.
...
6. Click **Confirm** in the warning pop-up.
7. Once the page reloads, **Is PayPal Checkout Flow Enabled?** is set to **Yes**. The flow is successfully enabled for your store.

**NOTE:** The PayPal Checkout Flow uses the `Bold_Checkout` module's "Parallel Checkout" Checkout type. Enabling the PayPal Checkout Flow in the Adobe Commerce admin changes the store's **Bold Checkout Type** to "Parallel".
